...
class proto_server_t {
  public:
    proto_server_t(uint16_t port);
    ~proto_server_t();

    bool start();

    void broadcast(const proto_message_t& message);
...
template<class proto_message_t>
proto_server_t<proto_message_t>::proto_server_t(uint16_t port)
...
// TODO move send from connection to here....

template<class proto_message_t>
void proto_server_t<proto_message_t>::broadcast(const proto_message_t& message) {
...
template<class proto_message_t>
const std::shared_ptr<common::owned_message_t<proto_message_t>> proto_server_t<proto_message_t>::getEvent() {
    ...
    if (incomingMessage->msg.ParseFromCodedStream(&cis)) {
        if (!cis.ConsumedEntireMessage()) {
            std::cout << "[Server] Could not parse entire message" << std::endl;
            throw; // TODO
        }
    } else {
        std::cout << "[Server] Message could not be processed: "
    ...
    _acceptor.async_accept(
        [this](std::error_code ec, boost::asio::ip::tcp::socket socket) {
            if (!ec) {
                ...
                std::cout << "[Server] Error occured while accepting client: " << ec.message() << std::endl;
            }
